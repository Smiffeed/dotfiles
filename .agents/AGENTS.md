...
- **Niri and Noctalia Shell Restarts**: If you reload Niri (`niri msg action load-config-file`), it may disrupt the Noctalia shell layer (which draws the wallpaper and top bar). If you ever need to restart Noctalia, you MUST do so through Niri's spawn command (`niri msg action spawn -- qs -c noctalia-shell`) rather than a raw background bash command, to ensure it inherits the correct Wayland environment.
- **Obsidian Notes Location**: Whenever the user instructs you to "note in obsidian" or save a note for them, always write the markdown file to the `~/Workspaces/Obsidian-Notes` directory.
